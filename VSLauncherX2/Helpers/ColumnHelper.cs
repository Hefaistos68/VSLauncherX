...
namespace VSLauncher.Helpers
{
	...
	public class ColumnHelper
	{
		...
		public static object GetImageNameForFile(object row)
		{
			if (row is VsFolder)
			{
				return App.VisualStudioFileIcons32.GetIcon("Folder");
			}

			if (row is VsSolution s)
			{
				return s.Warning ? App.VisualStudioFileIcons32.GetIcon("Warning") : App.VisualStudioFileIcons32.GetIcon("Solution");
			}

			if (row is VsProject p)
			{
				return p.Warning ? App.VisualStudioFileIcons32.GetIcon("Warning") : App.VisualStudioFileIcons32.GetIcon(p.ProjectType);
			}
			return string.Empty;
		}
		...
		public static object GetImageNameForFileImport(object row)
		{
			if (row is VsFolder)
			{
				return App.VisualStudioFileIcons16.GetIcon("Folder");
			}

			if (row is VsSolution s)
			{
				return s.Warning ? App.VisualStudioFileIcons16.GetIcon("Warning") : App.VisualStudioFileIcons16.GetIcon("Solution");
			}

			if (row is VsProject p)
			{
				return p.Warning ? App.VisualStudioFileIcons16.GetIcon("Warning") : App.VisualStudioFileIcons16.GetIcon(p.ProjectType);
			}
			return string.Empty;
		}
		...
		public static object GetImageNameForMru(object row)
		{
			if (row is VsFolder f)
			{
				return f.Icon is null ? App.VisualStudioFileIcons32.GetIcon(name: "Folder") : f.Icon;
			}
		...
		internal static object? GetImageForGit(object rowObject)
		{
			...
			if (rowObject is VsItem s)
			{
				...
				if (s.Status == "*")
				{
					return null; //App.Resources.GitDirty2;
				}
				else if (s.Status == "!")
				{
					return null; // Resources.GitClean2;
				}
			}
